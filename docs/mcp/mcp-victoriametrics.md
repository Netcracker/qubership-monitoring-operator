...
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------- |
| `VM_INSTANCE_ENTRYPOINT`   | Root URL of the VictoriaMetrics API. For in-cluster MCP use the direct `VMSingle` URL. For local MCP, the external `VMAuth` or `VMSingle` HTTPRoute or Ingress URL can be used. If `VMCluster` is installed, then `VMSelect` HTTPRoute or Ingress URL can be used. | Yes      |
