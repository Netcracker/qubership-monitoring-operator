...
The Prometheus Custom Resource (CR) allows to provide settings for Prometheus that are applied during
the start of a Prometheus instance.

This custom resource fully describes the Prometheus instance. Prometheus-operator discovers all `Prometheus` CRs
in the namespace where Monitoring (and prometheus-operator in particular) is deployed.
...
The `Alertmanager` Custom Resource (CR) allows to provide settings for Alertmanager that are applied while
starting the Alertmanager instance.
...
* For each separated `Alertmanager`, the CR creates a new Prometheus StatefulSet. This StatefulSet has the
  name generated by a `alertmanager-<CR_name>-0` mask. If more than one replicas are specified in
  the `Alertmanager` CR, the replicas are also specified in the associated StatefulSet.
