...
`/tmp`. The chart treats an empty `grafana.PAAS_PLATFORM` as Kubernetes unless
the OpenShift SCC API is available. Set it explicitly to `KUBERNETES` when API
discovery is unavailable and the pod must be pinned to UID and GID `1000`.
...
| `existingSecret.name`           | Existing Secret that contains the Grafana service account token. The selected key is mounted as a file.                                                                                                            | `""`                                                                 |
| `existingSecret.key`            | Key in `existingSecret.name` that contains the token and is projected to the token file.                                                                                                                           | `token`                                                              |
| `serviceAccountToken`           | Inline Grafana service account token. If set, Helm creates a Secret and mounts it as a file. Prefer `existingSecret` for production.                                                                               | `""`                                                                 |
...
| `basicAuth.password`            | Inline Grafana password for basic auth. Helm creates a Secret and references it from the Deployment. Prefer `basicAuth.passwordSecret` for production.                                                             | `""`                                                                 |
| `basicAuth.passwordSecret.name` | Existing Secret that contains the Grafana basic auth password.                                                                                                                                                     | `""`                                                                 |
| `basicAuth.passwordSecret.key`  | Key in `basicAuth.passwordSecret.name` that contains the password.                                                                                                                                                 | `password`                                                           |
...
| `allowedHosts`                  | Additional HTTP `Host` headers rendered through `tpl` and passed as `--allowed-hosts`. `localhost:<targetPort>`, the Pod IP, and hosts from enabled HTTPRoute or Ingress configuration are included automatically. | `[]`                                                                 |
...
sources are configured. In either case, the chart projects only the selected
Secret key to `/var/run/secrets/mcp-grafana/token` and sets
`GRAFANA_SERVICE_ACCOUNT_TOKEN_FILE` to that path. The token value is not
placed in the container environment. Upstream reads the file for every request,
so projected Secret updates are used without restarting the pod.

`basicAuth.passwordSecret.name` and `basicAuth.password` are also mutually
exclusive. An inline password is stored in a generated Kubernetes Secret named
`<mcp-name>-basic-auth`; it is never rendered in the Deployment. This remains
an environment-based integration because upstream `mcp-grafana` does not yet
support reading the Grafana password directly from a file. Changing an inline
password updates a pod-template checksum and restarts the MCP pod. After
updating an existing password Secret in place, restart the Deployment so the
container reads the new value.
...
to `allowedHosts` explicitly. The chart also adds each Pod's IP through the
Kubernetes Downward API so that Prometheus or `vmagent` can scrape `/metrics`
through a ServiceMonitor without disabling Host validation.
