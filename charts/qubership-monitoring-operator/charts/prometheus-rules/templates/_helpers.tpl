  ...
  rules:
    ...
    KubernetesVolumeFullInFourDays:
      ...
      annotations:
        ...
        description: "{{ "{{" }} $labels.namespace {{ "}}" }}/{{ "{{" }} $labels.persistentvolumeclaim {{ "}}" }} is expected to fill up within four days. Currently {{ "{{" }} $value | humanize {{ "}}" }} percent is available.\n  VALUE = {{ "{{" }} $value {{ "}}" }}\n  LABELS: {{ "{{" }} $labels {{ "}}" }}"
  ...
  rules:
    NodeDiskUsageIsMoreThanWarningThreshold:
      annotations:
        ...
        summary: "Disk usage on node > 70 percent (instance {{ "{{" }} $labels.node {{ "}}" }})"
    ...
    NodeDiskUsageIsMoreThanCriticalThreshold:
      annotations:
        description: "Node {{ "{{" }} $labels.node {{ "}}" }} disk usage of {{ "{{" }} $labels.mountpoint {{ "}}" }} is\n VALUE = {{ "{{" }} $value {{ "}}" }} percent"
        summary: "Disk usage on node > 90 percent (instance {{ "{{" }} $labels.node {{ "}}" }})"
...
SelfMonitoring:
  ...
  rules:
    DiskRunsOutOfSpaceIn3Days:
      expr: sum(vm_free_disk_space_bytes) without(path) / ((rate(vm_rows_added_to_storage_total[1d]) - sum(rate(vm_deduplicated_samples_total[1d])) without (type)) * (sum(vm_data_size_bytes{type!~"indexdb.*"}) without(type) / sum(vm_rows{type!~"indexdb.*"}) without(type)) + rate(vm_new_timeseries_created_total[1d]) * (sum(vm_data_size_bytes{type="indexdb/file"}) / sum(vm_rows{type="indexdb/file"}))) < 3 * 24 * 3600 > 0
      for: 30m
      labels:
        severity: critical
      annotations:
    ...
    NodeBecomesReadonlyIn3Days:
      expr: sum(vm_free_disk_space_bytes - vm_free_disk_space_limit_bytes) without(path) / ((rate(vm_rows_added_to_storage_total[1d]) - sum(rate(vm_deduplicated_samples_total[1d])) without (type)) * (sum(vm_data_size_bytes{type!~"indexdb.*"}) without(type) / sum(vm_rows{type!~"indexdb.*"}) without(type)) + rate(vm_new_timeseries_created_total[1d]) * (sum(vm_data_size_bytes{type="indexdb/file"}) / sum(vm_rows{type="indexdb/file"}))) < 3 * 24 * 3600 > 0
      for: 30m
      labels:
        severity: warning
      annotations:
    ...
    DiskRunsOutOfSpace:
      expr: sum(vm_data_size_bytes) by(job, instance) / ( sum(vm_free_disk_space_bytes) by(job, instance) + sum(vm_data_size_bytes) by(job, instance)) > 0.8
      for: 30m
      labels:
        severity: critical
      annotations:
        ...
        description: "Disk utilisation on instance {{ "{{" }} $labels.instance {{ "}}" }} is more than 80 percent.\n
          Having less than 20 percent of free disk space could cripple merges processes and overall performance.
    ...
    RPCErrors:
      expr: (sum(increase(vm_rpc_connection_errors_total[5m])) by(job, instance) + sum(increase(vm_rpc_dial_errors_total[5m])) by(job, instance) + sum(increase(vm_rpc_handshake_errors_total[5m])) by(job, instance)) > 0
      for: 15m
      labels:
        severity: warning
        show_at: dashboard
    ...
    TooHighChurnRate:
      expr: (sum(rate(vm_new_timeseries_created_total[5m])) by(job) / sum(rate(vm_rows_inserted_total[5m])) by(job)) > 0.1
      for: 15m
      labels:
        severity: warning
      annotations:
    ...
    TooHighChurnRate24h:
      expr: sum(increase(vm_new_timeseries_created_total[24h])) by(job) > (sum(vm_cache_entries{type="storage/hour_metric_ids"}) by(job) * 3)
      for: 15m
      labels:
        severity: warning
      annotations:
    ...
    TooHighSlowInsertsRate:
      expr: (sum(rate(vm_slow_row_inserts_total[5m])) by(job) / sum(rate(vm_rows_inserted_total[5m])) by(job)) > 0.05
      for: 15m
      labels:
        severity: warning
      annotations:
    ...
    VminsertVmstorageConnectionIsSaturated:
      ...
      annotations:
        ...
        description: "The connection between vminsert (instance {{ "{{" }} $labels.instance {{ "}}" }}) and vmstorage (instance {{ "{{" }} $labels.addr {{ "}}" }})
          is saturated by more than 90 percent and vminsert won't be able to keep up.\n
    ...
    TooHighMemoryUsage:
      ...
      labels:
        severity: critical
      annotations:
        summary: "It is more than 80 percent of memory used by {{ "{{" }} $labels.job {{ "}}" }}({{ "{{" }} $labels.instance {{ "}}" }})"
        description: |
    ...
    TooHighCPUUsage:
      ...
      labels:
        severity: critical
      annotations:
        summary: "More than 90 percent of CPU is used by {{ "{{" }} $labels.job {{ "}}" }}({{ "{{" }} $labels.instance {{ "}}" }}) during the last 5m"
        description: >
    ...
    RemoteWriteConnectionIsSaturated:
      ...
      annotations:
        ...
        description: "The remote write connection between vmagent {{ "{{" }} $labels.job {{ "}}" }} (instance {{ "{{" }} $labels.instance {{ "}}" }}) and destination {{ "{{" }} $labels.url {{ "}}" }}
          is saturated by more than 90 percent and vmagent won't be able to keep up.\n
    ...
    PersistentQueueForWritesIsSaturated:
      ...
      annotations:
        ...
        description: "Persistent queue writes for vmagent {{ "{{" }} $labels.job {{ "}}" }} (instance {{ "{{" }} $labels.instance {{ "}}" }})
          are saturated by more than 90 percent and vmagent won't be able to keep up with flushing data on disk. 
    ...
    PersistentQueueForReadsIsSaturated:
      ...
      annotations:
        ...
        description: "Persistent queue reads for vmagent {{ "{{" }} $labels.job {{ "}}" }} (instance {{ "{{" }} $labels.instance {{ "}}" }})
          are saturated by more than 90 percent and vmagent won't be able to keep up with reading data from the disk. 
          In this case, consider to decrease load on the vmagent or improve the disk throughput."
    ...
    SeriesLimitHourReached:
      ...
      annotations:
        ...
        summary: "Instance {{ "{{" }} $labels.instance {{ "}}" }} reached 90 percent of the limit"
