...
func TestHandlerReturnsDiscoveryError(t *testing.T) {
	...
	dc := failingDiscovery()
	reconciler := NewNodeExporterReconciler(c, scheme, dc)
	cr := &monv1.PlatformMonitoring{ObjectMeta: metav1.ObjectMeta{Name: "platformmonitoring", Namespace: "monitoring"}}

	err := reconciler.handleDaemonSet(cr)

	require.Error(t, err)
	err = c.Get(context.Background(), types.NamespacedName{Name: utils.NodeExporterComponentName, Namespace: "monitoring"}, &appsv1.DaemonSet{})
...
func TestDeleteDoesNotDependOnDiscovery(t *testing.T) {
	scheme := runtime.NewScheme()
	require.NoError(t, appsv1.AddToScheme(scheme))
	existing := &appsv1.DaemonSet{ObjectMeta: metav1.ObjectMeta{Name: utils.NodeExporterComponentName, Namespace: "monitoring"}}
	c := fake.NewClientBuilder().WithScheme(scheme).WithObjects(existing).Build()
	dc := failingDiscovery()
	reconciler := NewNodeExporterReconciler(c, scheme, dc)
	cr := &monv1.PlatformMonitoring{ObjectMeta: metav1.ObjectMeta{Name: "platformmonitoring", Namespace: "monitoring"}}

	require.NoError(t, reconciler.deleteDaemonSet(cr))

	err := c.Get(context.Background(), types.NamespacedName{Name: utils.NodeExporterComponentName, Namespace: "monitoring"}, &appsv1.DaemonSet{})
	assert.True(t, apierrors.IsNotFound(err), "the resource must be deleted")
	require.NoError(t, reconciler.deleteDaemonSet(cr), "a missing resource is not an error")
}
