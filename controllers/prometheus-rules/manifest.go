...
func prepareOverrideConfigMap(cr *v1beta1.PlatformMonitoring) map[string]map[string]*v1beta1.PrometheusRule {
	// Init map with info about chosen groups and overridden rules from CR
	overrideConfigMap := make(map[string]map[string]*v1beta1.PrometheusRule)
	...
	for _, group := range cr.Spec.PrometheusRules.RuleGroups {
		...
		if overrideConfigMap[group] == nil {
			overrideConfigMap[group] = make(map[string]*v1beta1.PrometheusRule)
		}
...
func prometheusRules(cr *v1beta1.PlatformMonitoring) (*promv1.PrometheusRule, error) {
