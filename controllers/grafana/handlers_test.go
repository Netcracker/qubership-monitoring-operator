...
func TestApplyGrafanaDesiredStateUpdatesChangedAnnotation(t *testing.T) {
	desired, err := grafanaWithDefaultSources(grafanaComparisonPlatformMonitoring(map[string]string{"example.com/key": "desired"}))
	require.NoError(t, err)
	live := desired.DeepCopy()
...
func TestApplyGrafanaDesiredStateRemovesStaleAnnotation(t *testing.T) {
	desired, err := grafanaWithDefaultSources(grafanaComparisonPlatformMonitoring(nil))
	require.NoError(t, err)
	live := desired.DeepCopy()
