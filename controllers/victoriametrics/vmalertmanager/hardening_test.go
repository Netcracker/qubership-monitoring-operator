...
func TestManifestRejectsConflictingHardeningSettings(t *testing.T) {
	tests := map[string]func(spec *monv1.VmAlertManager){
		"root user": func(spec *monv1.VmAlertManager) {
			spec.SecurityContext = &monv1.SecurityContext{RunAsUser: ptr.To[int64](0)}
		},
		"reserved volume name": func(spec *monv1.VmAlertManager) {
			spec.Volumes = []corev1.Volume{{
				Name:         utils.TmpVolumeMount().Name,
				VolumeSource: corev1.VolumeSource{Secret: &corev1.SecretVolumeSource{SecretName: "creds"}},
			}}
		},
		"privileged container": func(spec *monv1.VmAlertManager) {
	...
	for name, configure := range tests {
		t.Run(name, func(t *testing.T) {
			cr := hardeningCR()
			configure(&cr.Spec.Victoriametrics.VmAlertManager)
...
func TestManifestReturnsDiscoveryError(t *testing.T) {
	reconciler := NewVmAlertManagerReconciler(nil, runtime.NewScheme(), failingDiscovery())

	m, err := vmAlertManager(reconciler, hardeningCR())
...
func TestDeleteVMAlertmanagerDoesNotDependOnDiscovery(t *testing.T) {
	scheme := runtime.NewScheme()
	require.NoError(t, vmetricsv1b1.AddToScheme(scheme))
	existing := &vmetricsv1b1.VMAlertmanager{ObjectMeta: metav1.ObjectMeta{Name: "k8s", Namespace: "monitoring"}}
	controllerClient := fake.NewClientBuilder().WithScheme(scheme).WithObjects(existing).Build()
	reconciler := NewVmAlertManagerReconciler(controllerClient, scheme, failingDiscovery())

	require.NoError(t, reconciler.deleteVmAlertManager(hardeningCR()))

	err := controllerClient.Get(context.Background(), types.NamespacedName{Name: "k8s", Namespace: "monitoring"}, &vmetricsv1b1.VMAlertmanager{})
	assert.True(t, apierrors.IsNotFound(err), "the resource must be deleted")
	require.NoError(t, reconciler.deleteVmAlertManager(hardeningCR()), "a missing resource is not an error")
}
