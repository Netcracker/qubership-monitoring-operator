...
func TestManifestRejectsConflictingHardeningSettings(t *testing.T) {
	...
	for name, configure := range tests {
		t.Run(name, func(t *testing.T) {
			cr := hardeningCR()
			configure(&cr.Spec.Victoriametrics.VmAuth)
...
func TestManifestReturnsDiscoveryError(t *testing.T) {
	reconciler := NewVmAuthReconciler(nil, runtime.NewScheme(), failingDiscovery())

	m, err := vmAuth(reconciler, hardeningCR())
...
func TestDeleteVMAuthDoesNotDependOnDiscovery(t *testing.T) {
	scheme := runtime.NewScheme()
	require.NoError(t, vmetricsv1b1.AddToScheme(scheme))
	existing := &vmetricsv1b1.VMAuth{ObjectMeta: metav1.ObjectMeta{Name: "k8s", Namespace: "monitoring"}}
	controllerClient := fake.NewClientBuilder().WithScheme(scheme).WithObjects(existing).Build()
	reconciler := NewVmAuthReconciler(controllerClient, scheme, failingDiscovery())

	require.NoError(t, reconciler.deleteVmAuth(hardeningCR()))

	err := controllerClient.Get(context.Background(), types.NamespacedName{Name: "k8s", Namespace: "monitoring"}, &vmetricsv1b1.VMAuth{})
	assert.True(t, apierrors.IsNotFound(err), "the resource must be deleted")
	require.NoError(t, reconciler.deleteVmAuth(hardeningCR()), "a missing resource is not an error")
}
