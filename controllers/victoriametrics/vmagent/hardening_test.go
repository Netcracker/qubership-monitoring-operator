...
func TestManifestRejectsConflictingHardeningSettings(t *testing.T) {
	tests := map[string]func(spec *monv1.VmAgent){
		"root user": func(spec *monv1.VmAgent) {
			spec.SecurityContext = &monv1.SecurityContext{RunAsUser: ptr.To[int64](0)}
		},
		"privileged container": func(spec *monv1.VmAgent) {
	...
	for name, configure := range tests {
		t.Run(name, func(t *testing.T) {
			cr := hardeningCR()
			configure(&cr.Spec.Victoriametrics.VmAgent)
...
func TestManifestReturnsDiscoveryError(t *testing.T) {
	reconciler := NewVmAgentReconciler(nil, runtime.NewScheme(), failingDiscovery())

	m, err := vmAgent(reconciler, hardeningCR())
...
func TestDeleteVMAgentDoesNotDependOnDiscovery(t *testing.T) {
	scheme := runtime.NewScheme()
	require.NoError(t, vmetricsv1b1.AddToScheme(scheme))
	existing := &vmetricsv1b1.VMAgent{ObjectMeta: metav1.ObjectMeta{Name: "k8s", Namespace: "monitoring"}}
	controllerClient := fake.NewClientBuilder().WithScheme(scheme).WithObjects(existing).Build()
	reconciler := NewVmAgentReconciler(controllerClient, scheme, failingDiscovery())

	require.NoError(t, reconciler.deleteVmAgent(hardeningCR()))

	err := controllerClient.Get(context.Background(), types.NamespacedName{Name: "k8s", Namespace: "monitoring"}, &vmetricsv1b1.VMAgent{})
	assert.True(t, apierrors.IsNotFound(err), "the resource must be deleted")
	require.NoError(t, reconciler.deleteVmAgent(hardeningCR()), "a missing resource is not an error")
}
