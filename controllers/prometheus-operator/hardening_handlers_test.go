...
func TestHandlerReturnsDiscoveryError(t *testing.T) {
	...
	dc := failingDiscovery()
	reconciler := NewPrometheusOperatorReconciler(c, scheme, dc)
	...
	require.Error(t, err)
	err = c.Get(context.Background(), types.NamespacedName{Name: utils.PrometheusOperatorComponentName, Namespace: "monitoring"}, &appsv1.Deployment{})
...
func TestDeleteDoesNotDependOnDiscovery(t *testing.T) {
	scheme := runtime.NewScheme()
	require.NoError(t, appsv1.AddToScheme(scheme))
	existing := &appsv1.Deployment{ObjectMeta: metav1.ObjectMeta{Name: utils.PrometheusOperatorComponentName, Namespace: "monitoring"}}
	c := fake.NewClientBuilder().WithScheme(scheme).WithObjects(existing).Build()
	dc := failingDiscovery()
	reconciler := NewPrometheusOperatorReconciler(c, scheme, dc)
	cr := &monv1.PlatformMonitoring{ObjectMeta: metav1.ObjectMeta{Name: "platformmonitoring", Namespace: "monitoring"}}

	require.NoError(t, reconciler.deletePrometheusOperatorDeployment(cr))

	err := c.Get(context.Background(), types.NamespacedName{Name: utils.PrometheusOperatorComponentName, Namespace: "monitoring"}, &appsv1.Deployment{})
	assert.True(t, apierrors.IsNotFound(err), "the resource must be deleted")
	require.NoError(t, reconciler.deletePrometheusOperatorDeployment(cr), "a missing resource is not an error")
}
...
func TestManifestRejectsRootUser(t *testing.T) {
	cr := &monv1.PlatformMonitoring{
		...
		Spec:       monv1.PlatformMonitoringSpec{Prometheus: &monv1.Prometheus{Operator: monv1.PrometheusOperator{SecurityContext: &monv1.SecurityContext{RunAsUser: ptr.To[int64](0)}}}},
	}
