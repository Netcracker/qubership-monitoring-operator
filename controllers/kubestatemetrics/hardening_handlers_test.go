...
func TestHandlerReturnsDiscoveryError(t *testing.T) {
	...
	dc := failingDiscovery()
	reconciler := NewKubeStateMetricsReconciler(c, scheme, dc)
	...
	require.Error(t, err)
	err = c.Get(context.Background(), types.NamespacedName{Name: utils.KubestatemetricsComponentName, Namespace: "monitoring"}, &appsv1.Deployment{})
...
func TestDeleteDoesNotDependOnDiscovery(t *testing.T) {
	scheme := runtime.NewScheme()
	require.NoError(t, appsv1.AddToScheme(scheme))
	existing := &appsv1.Deployment{ObjectMeta: metav1.ObjectMeta{Name: utils.KubestatemetricsComponentName, Namespace: "monitoring"}}
